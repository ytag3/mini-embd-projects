 ...
 * in the root directory of this sJoftware component.
...
void init_task_stack(void){
	...
	user_tasks[0].task_handler = TASK_RUNNING_STATE;
	user_tasks[1].task_handler = TASK_RUNNING_STATE;
	user_tasks[2].task_handler = TASK_RUNNING_STATE;
	user_tasks[3].task_handler = TASK_RUNNING_STATE;
	user_tasks[4].task_handler = TASK_RUNNING_STATE;

	uint32_t *pPSP; //walk down each private stack to store values
...
void update_next_task(void){
	...
	uint8_t state = TASK_BLOCKED_STATE; //initialized to blocked, so stays blocked unless updated otherwise

	for(int i = 0; i < (MAX_TASKS); i++){ //check for blocked tasks
		current_task++;
		current_task = current_task % MAX_TASKS; //between 1=4
		state = user_tasks[i].current_state;
		if ((state == TASK_RUNNING_STATE) && (current_task !=0)) break; //if user task is ready, then....
	}
	if (state != TASK_RUNNING_STATE) current_task = 0; //if still blocked
